...
func NewSyreenApp(
	...
	// v1.2.1: Fix EVM CLI nonce auto-query
	app.UpgradeKeeper.SetUpgradeHandler("v1.2.1", func(ctx context.Context, plan upgradetypes.Plan, fromVM module.VersionMap) (module.VersionMap, error) {
		sdkCtx := sdk.UnwrapSDKContext(ctx)
		sdkCtx.Logger().Info("running v1.2.1 upgrade handler — EVM nonce fix", "height", sdkCtx.BlockHeight())
		return app.mm.RunMigrations(ctx, app.configurator, fromVM)
	})

	// v1.2.2: Correct EVM CLI nonce query (accounts for ante handler sequence increment)
	app.UpgradeKeeper.SetUpgradeHandler("v1.2.2", func(ctx context.Context, plan upgradetypes.Plan, fromVM module.VersionMap) (module.VersionMap, error) {
		sdkCtx := sdk.UnwrapSDKContext(ctx)
		sdkCtx.Logger().Info("running v1.2.2 upgrade handler — EVM nonce fix corrected", "height", sdkCtx.BlockHeight())
		return app.mm.RunMigrations(ctx, app.configurator, fromVM)
	})

	// v1.2.3: Identity query graceful response for missing records
	app.UpgradeKeeper.SetUpgradeHandler("v1.2.3", func(ctx context.Context, plan upgradetypes.Plan, fromVM module.VersionMap) (module.VersionMap, error) {
		sdkCtx := sdk.UnwrapSDKContext(ctx)
		sdkCtx.Logger().Info("running v1.2.3 upgrade handler — identity query fix", "height", sdkCtx.BlockHeight())
		return app.mm.RunMigrations(ctx, app.configurator, fromVM)
	})
	...
	// v1.2.5: Fix EVM gas estimation (remove SDK gas capping) + fix EVM code persistence (StateDB context restore)
	app.UpgradeKeeper.SetUpgradeHandler("v1.2.5", func(ctx context.Context, plan upgradetypes.Plan, fromVM module.VersionMap) (module.VersionMap, error) {
		sdkCtx := sdk.UnwrapSDKContext(ctx)
		sdkCtx.Logger().Info("running v1.2.5 upgrade handler — EVM gas + code persistence fixes", "height", sdkCtx.BlockHeight())
		return app.mm.RunMigrations(ctx, app.configurator, fromVM)
	})
