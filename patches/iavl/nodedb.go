...
// NOTE: We cannot rely on db.Has() alone because goleveldb's Get (and thus Has)
// returns nil for keys stored with empty []byte{} values. We use an iterator
// as a fallback to detect such keys.
func (ndb *nodeDB) hasVersion(version int64) (bool, error) {
	key := nodeKeyFormat.Key(GetRootKey(version))
	has, err := ndb.db.Has(key)
	if err != nil {
		return false, err
	}
	if has {
	...
	// Fallback: db.Has returns false for keys with empty values in goleveldb.
	// Use a point iterator to check if the key truly exists.
	itr, err := ndb.db.Iterator(key, append(key, 0xFF))
	if err != nil {
		return false, err
	}
	defer itr.Close()
...
func (ndb *nodeDB) hasLegacyVersion(version int64) (bool, error) {
	...
	// Fallback for empty-value keys (see hasVersion comment).
	itr, err := ndb.db.Iterator(key, append(key, 0xFF))
	if err != nil {
		return false, err
	}
...
func (ndb *nodeDB) GetRoot(version int64) ([]byte, error) {
	...
	formattedKey := nodeKeyFormat.Key(rootKey)
	val, err := ndb.db.Get(formattedKey)
	if err != nil {
		return nil, err
	}
	if val == nil {
		// goleveldb returns nil for keys with empty values ([]byte{}).
		// Use an iterator to check if the key exists with an empty value,
		// which indicates an empty root (written by SaveEmptyRoot).
		itr, itrErr := ndb.db.Iterator(formattedKey, append(formattedKey, 0xFF))
		if itrErr == nil {
			if itr.Valid() && bytes.Equal(itr.Key(), formattedKey) {
				// Key exists with empty value -> empty root
				itr.Close()
				return nil, nil
			}
			itr.Close()
	...
	if len(val) == 0 || bytes.Equal(val, []byte{0x00}) { // empty root (old or new sentinel format)
		return nil, nil
	}
...
// NOTE: We write a single-byte sentinel (0x00) instead of an empty []byte{}
// because goleveldb's Get returns nil for keys with empty values (due to
// append(nil, []byte{}...) returning nil in Go). This would cause
// GetRoot to think the key does not exist, breaking restart.
