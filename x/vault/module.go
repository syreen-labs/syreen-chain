...
func (am AppModule) InitGenesis(ctx sdk.Context, _ codec.JSONCodec, data json.RawMessage) {
	...
	if err := json.Unmarshal(data, &gs); err != nil {
		ctx.Logger().Error("FATAL: malformed vault genesis JSON — halting startup", "module", types.ModuleName, "error", err)
		panic(fmt.Errorf("failed to unmarshal %s genesis: %w", types.ModuleName, err))
	}
	for _, v := range gs.Vaults { am.keeper.SetVault(ctx, v) }
