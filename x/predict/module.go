...
func (am AppModule) InitGenesis(ctx sdk.Context, _ codec.JSONCodec, data json.RawMessage) {
	...
	for _, r := range gs.Resolutions {
		am.keeper.SetResolution(ctx, r)
	}
	return
}

func (am AppModule) ExportGenesis(ctx sdk.Context, _ codec.JSONCodec) json.RawMessage {
	gs := types.GenesisState{Markets: am.keeper.GetAllMarkets(ctx)}
...
func (am AppModule) BeginBlock(goCtx context.Context) error {
	...
	if !config.IsModuleEnabled("predict") {
		return nil
	}
	// Auto-close markets past their resolution block
	am.keeper.AutoCloseExpiredMarkets(ctx)
	return nil
}
