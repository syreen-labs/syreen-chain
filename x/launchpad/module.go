...
func (am AppModule) InitGenesis(ctx sdk.Context, _ codec.JSONCodec, data json.RawMessage) {
	...
	for _, l := range gs.Launches { am.keeper.SetLaunch(ctx, l) }
	for _, c := range gs.Contributions { am.keeper.SetContribution(ctx, c) }
	for _, v := range gs.Vestings { am.keeper.SetVesting(ctx, v) }
	return
}

func (am AppModule) ExportGenesis(ctx sdk.Context, _ codec.JSONCodec) json.RawMessage {
	gs := types.GenesisState{
		Launches:      am.keeper.GetAllLaunches(ctx),
		Contributions: am.keeper.GetAllContributions(ctx),
		Vestings:      am.keeper.GetAllVestings(ctx),
	}
...
func (am AppModule) BeginBlock(goCtx context.Context) error {
	...
	if !config.IsModuleEnabled("launchpad") {
		return nil
	}
	am.keeper.CheckAndFinalizeExpiredLaunches(ctx)
	return nil
}
